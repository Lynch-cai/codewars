...
function zombie_shootout(zombies, range, ammo) {
    let zombiesStart = Number(zombies)
    while (true) {
        if (range > 0){
            if(ammo >= 1){
                ammo -= 1
        ...
        if(range <= 0 && zombies >= 1){
            if (ammo >= 0 && ammo >= 0){
                return `You shot ${zombiesStart - zombies} zombies before being eaten: overwhelmed.`
            }
            else {
                return `You shot ${zombiesStart - zombies} zombies before being eaten: ran out of ammo.`
            }
        }
        else if(zombies == 0){
            return `You shot all ${zombiesStart} zombies.`
        }
    }
}
...
//     assert.strictEqual(zombie_shootout(100, 8, 200), "You shot 16 zombies before being eaten: overwhelmed.");
//     assert.strictEqual(zombie_shootout(50, 10, 8), "You shot 8 zombies before being eaten: ran out of ammo.");
//   });
// });