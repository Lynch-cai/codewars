...
export function nbMonths(startPriceOld: number, startPriceNew: number, savingperMonth: number, percentLossByMonth: number): number[] {
  if (startPriceOld < startPriceNew) {
    ...
    while (total <= 0) {
      day++
      const ratio = ((100-percentLossByMonth)/100) - (0.005 * Math.floor(day/2))
      oldprice = oldprice * ratio
      newprice = newprice * ratio
      total = oldprice + (savingperMonth * day) - newprice
    }
    return [day, Math.round(total)]
  }
  else {
